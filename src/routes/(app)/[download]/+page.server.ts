...
export const actions = {
    downloaded: async ({ params, request }) => {
        ...
        if (transferRecord.verification !== verification) {
            return { success: false, error: 'Invalid verification token' };
        }

        // Mark transfer as complete (clear anything unneeded to save space)
        await db.update(transfer).set({
            ...
            eq(transfer.id, transferRecord.id)
        ).catch((error) => {
            console.error('Database error:', error);
