...
export const rtcConfiguration: RTCConfiguration = {
    ...
    iceCandidatePoolSize: navigator.userAgent.toLowerCase().includes('firefox') ? 0 : 10,
...
function fixSdp(sdp: string): string {
    return sdp.replace(/a=fmtp:\d+ apt=\d+/g, '');
}
...
export class WebRTCSender extends WebRTCBase {
    ...
    private chunkSize: number = chunkSize; // Default to 1MB
        ...
        return new Promise((resolve, reject) => {
            ...
            fileReader.onload = async () => {
                ...
                try {
                    ...
                    const chunks = await this.processFileWithWorker(fileBuffer);
                    this.chunks = chunks;
    ...
    private processFileWithWorker(fileBuffer: ArrayBuffer): Promise<string[]> {
        return new Promise((resolve, reject) => {
            ...
            worker.postMessage({
                fileBuffer,
                chunkSize: this.chunkSize // Make sure chunkSize is accessible
            });

            worker.onmessage = (e) => {
                const { type, chunks, progress, error } = e.data;
                
                switch (type) {
                    case 'progress':
                        // Optional: Handle progress updates
                        console.log(`Processing: ${progress.toFixed(1)}%`);
                        this.events.onProcessingProgress?.(progress);
                        break;
    ...
    private async sendNextChunk(sequence: number): Promise<void> {
        if (!this.fileReader || !this.dataChannel || !this.file) {
        ...
        if (sequence >= this.chunks.length) {
            console.log('All chunks sent');
            this.events.onFileComplete?.();
            return;
        }

        const encodedChunk = this.chunks[sequence];
        const isLast = sequence === this.chunks.length - 1;
        ...
        const progress: TransferProgress = {
            bytesTransferred: Math.min((sequence + 1) * chunkSize, this.file.size),
...
export class WebRTCReceiver extends WebRTCBase {
    ...
    private async handleFileChunk(message: any): Promise<void> {
        ...
        if (this.transferComplete) {
            console.log('Transfer already marked complete, ignoring chunk.');
            return;
        }

        if (!this.writable) {
            console.error('Writable stream not initialized');
            return;
        }

        try {

            console.log('Writing chunk to stream, sequence:', message.sequence);
    ...
    private async completeFileTransfer(): Promise<void> {
        ...
        try {
            const response = await fetch('?/downloaded', {
                ...
                body: `verification=${encodeURIComponent(this.verification)}`
            });
            ...
            console.log('Server notified of file receipt successfully');
    ...
    public setMetaData(name: string, type: string, size: number, hash: string): void {
        ...
        new Promise(async () => {
            const url = URL.createObjectURL(await new Response(this.stream).blob());
            const a = document.createElement('a');
